...
to move-turtles          ; here the relocation decision, I might simplify further
  ...
  ask turtles [
   ...
   let peak_v ifelse-value (shape = "square") [val_con][val_lib]   ; value homophily threshold, varying between value orientation
    ...
    let M ifelse-value (shape = "square")[M_con][M_lib]
  ...
  ;    set id [who] of myself
      ...
      set uti-eth  utility xe n peak_e  M  ; ethnic utility computation (below the report)
      set uti-val  utility xv n peak_v  M   ; value utility computation (below the report)
...
to-report utility [sim tot peak M]   ; the three types of utility functions: threshold, single-peaked, linear + constant

 report ( ifelse-value (tot = 0) [0]
    [ifelse-value (sim <= (tot * peak))
   [ precision (sim / (tot * peak))  3
 ; ][ precision (  (2 - M) + ( ((M - 1) * (sim / tot)) / peak )   ) 3 ]
       ][ precision (  (2 - M) + ( ((M - 1) * sim) / (tot * peak) )   ) 3 ]
  ]
    )
