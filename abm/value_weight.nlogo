...
to move-turtles          ; here the relocation decision, I might simplify further
  ...
  ask turtles [
                                                                           ...
                                                                           ; weights attributed by value dominant preference [0,1]
   ...
   let trial random-float 1.00                                   ; random number to compare to probability to move to alternative
    ...
    ask options [
      ...
      set uti-eth  utility xe n peak_e    ; ethnic utility computation (below the report)
      set uti-val  utility xv n peak_v    ; value utility computation (below the report)
...
utility_function
utility_function
"threshold" "single-peaked" "symmetric" "linear-const"
1
...
Beta weight is attributed by value orientation and dominant preference:

conservatives: dom_con = ethncity
	dom_con = 1: maximize ethnic similarity [1], not care for value similarity [0]
	dom_con = 0.5: consider both ethnic and value similarity, each given weight  0.5

liberals: dom_lib = value
	dom_lib = 1: maximize value similarity [1], not care for ethnic similarity [0]
	dom_lib = 0.5: consider both ethnic and value similarity, each given weight  0.5



determinism (equivalent to beta): global parameter, meaning agents select the best option based on their determinism (max(determinism)) or totally random for determinism=0 (rum: randomness never totally disappear)
Within the choice, which dominant or secondary preference matters more depend on their weight (dominant = 0.5, both are considered, what differs is the utility (different concentration)
@#$#@#$#@
default
true
0
...
@#$#@#$#@
<experiments>
  <experiment name="basic_compl_2" repetitions="1" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    ...
    <steppedValueSet variable="dominant_con" first="0.5" step="0.1" last="1"/>
    <enumeratedValueSet variable="eth_con">
      <value value="1"/>
    </enumeratedValueSet>
    <steppedValueSet variable="val_con" first="0.5" step="0.1" last="1"/>
    <steppedValueSet variable="dominant_lib" first="0.5" step="0.1" last="1"/>
    <enumeratedValueSet variable="val_lib">
      <value value="1"/>
    </enumeratedValueSet>
    <steppedValueSet variable="eth_lib" first="0.5" step="0.1" last="1"/>
