...
* how by-product value segregation of conservatives can be reduced through: \par

  * lower concentration peaks \par
  
  * when also secondary preferences are taken into account --> extreme condition of integrationist preferences
  
    * a difference between value integrationist preferences of conservatives, this is more as robustness of the by-product, and ethnic integrationist preferences of liberals   \par
    
    * effect of randomness on these dynamics \par
    
* How results would change for ethnic asymmetric conditions \par
...
df_bsl %>% filter(ID == "bsl" & determinism == 20 & eth_con == 0.5) %>% ggplot(aes(x=val_lib, y=Segra_bsl, color = outcome, shape = orientation)) + geom_point(size = 1.5) + geom_line() +
  #  facet_wrap(~ ID, dir = "v") +
