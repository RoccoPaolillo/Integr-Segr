...
df <- df %>% select(-run) %>% group_by(ID,density,fraction_blue,circle_blue,circle_orange,dominant_con,M_con_dom,eth_con,M_con_sec,val_con,dominant_lib,M_lib_dom,val_lib,M_lib_sec,eth_lib,determinism, step) %>% summarise_all(mean) %>% gather(key = group_bsl, value = Segra_bsl, et_cl,et_sq,vl_cl,vl_sq,den_sq,den_cl)  %>%
